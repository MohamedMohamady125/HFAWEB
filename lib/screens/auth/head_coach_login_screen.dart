...
import '../coach/head_coach_only/head_coach_home_screen.dart';
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (result['success']) {
        ...
        final userRole = loginData['user_type'];

        if (userRole != 'head_coach') {
          _showError(
            _languageService.getLocalizedText(
              'Access denied. Head Coach login only.',
              'تم رفض الوصول. تسجيل دخول المدرب الرئيسي فقط.',
            ),
          );
          return;
        }
        ...
        if (mounted) {
          ...
          Navigator.of(context).pushAndRemoveUntil(
            MaterialPageRoute(
              builder: (context) => const RegularCoachHomeScreen(),
            ),
            (route) => false,
          );
        }
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      _languageService.getLocalizedText(
                                        'This portal is for Head Coaches only. Your session is encrypted and monitored for security.',
                                        'هذه البوابة للمدربين الرئيسيين فقط. جلستك مشفرة ومراقبة للأمان.',
                                      ),
                                      style: Theme.of(
                                        context,
                                      ).textTheme.bodyMedium?.copyWith(
        ...
        child: BackdropFilter(
          ...
          child: TextFormField(
            ...
            decoration: InputDecoration(
              hintText: hintText,
              hintStyle: const TextStyle(color: Color(0xFF108FFF)),
              prefixIcon: Icon(
                icon,
